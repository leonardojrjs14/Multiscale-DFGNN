...
def initialize_model_for_dataset(model: Module, dataset):
    """Initialize optional static model state derived from a processed graph.

    Ordinary single-scale models are returned unchanged.  A multiscale model
    receives the post-boundary-processing topology from the first sample; the
    same catchment graph is shared by every event and timestep in the dataset.
    """

    if not getattr(model, "requires_multiscale_hierarchy", False):
        return None
    ...
    if boundary_condition is not None:
        boundary_mask = getattr(boundary_condition, "boundary_nodes_mask", None)
    ...
    return model.get_hierarchy_summary()
