...
Check the estimated starting time:

```bash
squeue --start -j JOB_ID
```

Check available cluster partitions:
...
# Checking the Slurm Environment

To reproduce an experiment, record the Python environment used during training.
...
```bash
python -m pip freeze > requirements_cluster.txt
```

---

## Check PyTorch, PyG, and CUDA
...
Check the NVIDIA GPU environment:

```bash
nvidia-smi
```

Check loaded cluster modules:
...
Experiment environment information can be saved using:

```bash
mkdir -p environment_info

python --version > environment_info/python.txt 2>&1
which python > environment_info/python_path.txt
python -m pip freeze > environment_info/requirements.txt
nvidia-smi > environment_info/nvidia_smi.txt 2>&1
module list 2> environment_info/modules.txt
```

Save PyTorch information:

```bash
python - <<'PY' > environment_info/pytorch.txt
import torch
...
```text
saved_models/
└── MultiScaleDUALFloodGNN_<timestamp>.pt
```
...
On HPC systems, the login node may not have access to a GPU. GPU checks may need to be performed inside a GPU compute allocation.

---

## Check Which Model File Is Being Used

If multiple copies of the project exist:

```bash
python - <<'PY'
import inspect
from models.multiscale_dual_flood_gnn import MultiScaleDUALFloodGNN

print(inspect.getfile(MultiScaleDUALFloodGNN))
PY
```
