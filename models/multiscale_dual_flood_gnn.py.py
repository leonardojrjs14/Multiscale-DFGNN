...
class MultiScaleDUALFloodGNN(BaseModel):
    ...
    blocks.  Latent nodes and crossing latent edges are mean-pooled.

    On the upward path, coarse latent features are broadcast back to their
    finer children using the fixed hierarchy mapping, concatenated with the
    saved skip features, and mixed by one fusion MLP module before finer-scale
    message passing.  Model outputs always retain the original fine node and
    edge ordering.
    """
    ...
    def __init__(
        ...
        **base_model_kwargs,
        ...
        # Upward transfer is intentionally simple:
        #   1) broadcast each coarse parent latent back to its finer child nodes,
        #   2) concatenate it with the saved skip latent,
        #   3) use one fusion MLP module to mix both sources.
        #
        # There is NO separate learnable upsampling MLP.  The structural
        # upsampling itself is performed by the fixed hierarchy mapping.
    ...
    def forward(
        ...
        for level in reversed(range(self.num_scales - 1)):
            ...
            # NODE UPSAMPLING (non-learnable structural transfer):
            # Every finer node receives the latent vector of its coarse parent.
            #
            # Example:
            #   fine_to_coarse = [0, 0, 1, 1]
            #   coarse latents = [h0, h1]
            # gives:
            #   upsampled_nodes = [h0, h0, h1, h1]
            #
            # No MLP is applied here.  Learning happens later in the fusion MLP.
            upsampled_nodes = node_latent[fine_to_coarse]

            # EDGE UPSAMPLING (non-learnable structural transfer):
            # Crossing finer edges receive the latent of their corresponding
            # directed coarse edge.  Internal finer edges have no coarse-edge
            # parent, so their coarse contribution is zero; their local detail
            # is preserved by the skip connection.
            ...
            if debug_message_passing:
                ...
                print("Upsampling operation : fixed parent/edge lookup (NO MLP)")

            # CONCATENATE + ONE FUSION MLP:
            # The finer-scale skip latent preserves local detail, while the
            # upsampled parent latent provides broader-scale context.
            #
            # With hidden_features = 64 and skip_connections = True:
            #   skip          : [N_fine, 64]
            #   coarse parent : [N_fine, 64]
            #   concatenate   : [N_fine, 128]
            #   fusion MLP    : [N_fine, 128] -> [N_fine, 64]
            ...
            if debug_message_passing:
                ...
                print("Fusion operation     : ONE fusion MLP module")
